  ...
  fetchRecentCampaign() async {
    ...
    if (connection) {
      ...
      if (response.statusCode == 200) {
        ...
        recentCampaignList = data.donationRecent.data;
        notifyListeners();
      } else {
    ...
    if (connection) {
      ...
      if (response.statusCode == 200 &&
        ...
        if (isrefresh) {
          ...
          allRecentCampaign = [];
          allRecentCampaign = data.donationRecent.data;
          notifyListeners();
        } else {
          print('add new data');

          //else add more data to list
          for (int i = 0; i < data.donationRecent.data.length; i++) {
            allRecentCampaign.add(data.donationRecent.data[i]);
          }
          notifyListeners();
        }
