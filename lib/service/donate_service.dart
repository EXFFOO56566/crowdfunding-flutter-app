...
class DonateService with ChangeNotifier {
  ...
  // calculate tips after amount selected or written
  // calculateTipsAfterSelectedOrRritten() {
  //   if (tipsAmountType != 'percentage') {
  //     //fixed amount
  //     tips = defaultDonateAmount;
  //   } else {
  //     tips = (donationAmount.runtimeType == int
  //             ? donationAmount
  //             : int.parse(donationAmount) * chargeAmount) /
  ...
  calculateInitialDonationAmount() {
    if (minimumDonateAmount != 0) {
      donationAmount = minimumDonateAmount;
    } else {
