...
import 'dart:convert';
import 'package:fundorex/model/campaign_category_model.dart';
...
//============>
  ...
  fetchCampaignCategory(BuildContext context, {categoryId}) async {
    ...
    var response = await http.get(
        Uri.parse('$baseApi/user/support-tickets/ticket-departments'),
        headers: header);

    setDropdownLoadingStatus(false);

    if (response.statusCode == 200) {
      var data = CampaignCategoryModel.fromJson(jsonDecode(response.body));
      for (int i = 0; i < data.content.length; i++) {
        categoryDropdownList.add(data.content[i].name);
        categoryDropdownIndexList.add(data.content[i].id);
      }

      if (categoryId == null) {
        //if no categoryId id passed then show first one as selected
        selectedCategory = data.content[0].name;
        selectedCategoryId = data.content[0].id;
      } else {
        //select initial one based on category id of campaign
        for (int i = 0; i < data.content.length; i++) {
          if (data.content[i].id == categoryId) {
            selectedCategory = data.content[i].name;
            selectedCategoryId = data.content[i].id;
            break;
          }
        }
      }
