    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SmartRefresher(
          ...
          enablePullUp: true,
          enablePullDown:
              context.watch<AllEventsService>().currentPage > 1 ? false : true,
          onRefresh: () async {
            final result =
                await Provider.of<AllEventsService>(context, listen: false)
          ...
          child: SingleChildScrollView(
            child: Consumer<AllEventsService>(
              builder: (context, provider, child) => provider.hasData == true
                  ? Container(
                      padding: EdgeInsets.symmetric(
                          horizontal: screenPadding, vertical: 20),
                      child: Column(children: [
                        for (int i = 0; i < provider.allEventsList.length; i++)
                          EventCard(
                            imageLink: provider.allEventsList[i].image ??
                                placeHolderUrl,
                            title: provider.allEventsList[i].title ?? '',
                            ...
                            location: provider.allEventsList[i].venueLocation,
                          ),
                      ]),
                    )
                  : OthersHelper().showError(context, 'No events found'),
            ),
          ),
        ),
      ),
